...
import { TernaryMcpCall } from "./TernaryMcpCall";
...
function preprocessUnclosedTags(content: string): {
  ...
  const customTagNames = [
    ...
    // Include ternary-mcp-call so the block renders even if the stream ends without an explicit close tag.
    "ternary-mcp-call",
    "think",
  ];
...
function parseCustomTags(content: string): ContentPiece[] {
  ...
  // into the canonical <ternary-mcp-call> form so our parser can render them.
  const normalized = content
    .replace(/<\s*-?mcp-call(\s|>)/g, (m, g1) => `<ternary-mcp-call${g1}`)
    .replace(/<\s*\/\s*-?mcp-call\s*>/g, `</ternary-mcp-call>`);

  const { processedContent, inProgressTags } =
    preprocessUnclosedTags(normalized);
